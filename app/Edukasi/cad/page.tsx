...
export default function CurrencyDetailPage() {
  const currency = currencyDetails.cad;
      ...
      <main className="min-h-screen bg-[#1f3a32] text-white pt-[80px]">
        <section className="relative flex flex-col items-center justify-center text-center py-20 px-4 md:py-32 bg-linear-to-b from-[#0f1f19] via-[#14271f] to-[#0f1f19]">
          ...
          <div className="relative z-10 max-w-3xl">
            <h1 className="text-4xl md:text-5xl font-extrabold mb-6 text-[#f0c94e]">Kenali dan Pelajari Berbagai Mata Uang Dunia!</h1>
            <p className="text-lg md:text-xl text-[#d9d6bf]">Tingkatkan wawasan Anda tentang mata uang global—mulai dari simbol, kode ISO, hingga fakta menarik di baliknya.</p>
          </div>
        </section>
        ...
        <div className="bg-[#1a2d27] border border-[#D4AF37]/40 rounded-xl p-4 mt-12 container mx-auto max-w-6xl px-4">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="relative flex-1">
              ...
              <Input placeholder="Cari mata uang atau kode ISO..." className="bg-[#0C0C0C] border border-[#D4AF37]/40 text-white placeholder:text-gray-500/70 pl-11 h-9 rounded-md focus:ring-2 focus:ring-[#f0c94e]" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} onKeyDown={handleSearchKeyDown} />
            </div>
        ...
        <section className="pb-16 px-4 sm:px-8 flex justify-center">
          <div className="container mx-auto max-w-6xl bg-[#10261e] p-6 sm:p-10 rounded-xl shadow-2xl mt-[60px] relative z-20">
            ...
            <div className="text-center mb-10">
              <Image src={currency.flag} alt={currency.country} width={120} height={80} className="rounded-lg mx-auto mb-4 shadow-xl" priority />
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 sm:px-12 py-5 bg-[#0E2C27]/80 backdrop-blur-md shadow-sm text-white">
      ...
      <nav className="flex gap-4 sm:gap-8 text-xs sm:text-sm font-medium">
        ...
        <Link href="/konversi-mata-uang" className="hover:text-[#f0c94e]">Konversi</Link>
        <Link href="/dokumen-api" className="hover:text-[#f0c94e] hidden md:inline">API Docs</Link>
      </nav>
    </header>
  );
}
...
function Footer() {
  return (
    <footer className="bg-[#0E0E0E] py-10 px-4 sm:px-12 text-sm text-gray-300">
      ...
      <div className="text-center mt-10 py-4 text-gray-400 border-t border-[#0f251e]">© 2025 FinSight. All rights reserved.</div>
    </footer>
  );
}
    ...
    return (
      <div className="relative bg-[#10261e]/60 backdrop-blur-sm rounded-xl overflow-hidden shadow-xl transition-all duration-300 hover:scale-[1.04] hover:border-[#D4AF37]/50 hover:shadow-2xl">
        <div className="p-2 pt-0 pb-6">
          <Image src={item.image} alt={`Uang Kertas ${item.value} ${currency.code}`} width={300} height={150} className="w-full object-contain" loading="lazy" />
          <div className="bg-[#F5F2EC]/60 px-4 py-1 border-t border-[#D4AF37]/30 rounded-b-lg absolute bottom-0 left-2 right-2 z-10">
            <p className="text-center text-lg font-extrabold text-black tracking-wider">{currency.symbol}{item.value}</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="relative bg-[#0f1f19]/60 backdrop-blur-sm rounded-xl overflow-hidden shadow-xl transition-all duration-300 hover:scale-[1.04] hover:border-[#D4AF37]/50 hover:shadow-2xl">
      <div className="p-15 pb-0">
        <Image src={item.image} alt={`Koin ${item.value} ${currency.code}`} width={160} height={160} className="w-full object-contain rounded-full" loading="lazy" />
        <div className="bg-[#F5F2EC]/60 px-4 py-4 border-t border-[#D4AF37]/30 rounded-b-lg absolute bottom-0 left-15 right-15 z-10">
          <p className="text-center text-lg font-extrabold text-black tracking-wider">{item.value}</p>
        </div>
      </div>
    </div>
  );
...
const currencyDetails = {
  cad: {
    ...
    country: "Kanada",
    paperNotes: [
      { value: "5 CAD", image: "/currencies/cad/5cad.png" },
      { value: "10 CAD", image: "/currencies/cad/10cad-ver1.jpg" },
      { value: "20 CAD", image: "/currencies/cad/20cad.jpg" },
      { value: "50 CAD", image: "/currencies/cad/50cad.jpg" },
      { value: "100 CAD", image: "/currencies/cad/100cad.jpg" },
    ],
    coins: [
      { value: "1¢", image: "/currencies/cad/1cent.png" },
      { value: "5¢", image: "/currencies/cad/5cent.png" },
      { value: "10¢", image: "/currencies/cad/10cent.png" },
      { value: "25¢", image: "/currencies/cad/25cent.png" },
      { value: "50¢", image: "/currencies/cad/50cent.png" },
      { value: "$1 CAD", image: "/currencies/cad/1cad.png" },
      { value: "$2 CAD", image: "/currencies/cad/2cad.png" },
    ],
  },
};